    ...
    public class fsJsonParser {
        ...
        private fsResult TryParseNumber(out fsData data) {
            ...
            if (numberString.Contains(".") || numberString == "Infinity" || numberString == "-Infinity" || numberString == "NaN") {
                ...
                UInt64 intValue;
                if (UInt64.TryParse(numberString, NumberStyles.Any, CultureInfo.InvariantCulture, out intValue) == false)
                {
                    data = null;
                    return MakeFailure("Bad UInt64 format with " + numberString);
                }

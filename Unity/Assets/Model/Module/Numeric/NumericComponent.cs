...
namespace ETModel
{
    ...
    [ObjectSystem]
    public class NumericComponentStartSystem : StartSystem<NumericComponent>
    {
        public override void Start(NumericComponent self)
        {
            self.Start();
        }
    }
    ...
    public class NumericComponent: Component
	{
        ...
        public float Change(NumericType numericType, float changeValue)
        {
            float res = this.GetAsFloat(numericType) + changeValue;
            Set(numericType, res);
            return res;
        }
		...
		public void Update(NumericType numericType)
		{
			...
			Game.EventSystem.Run(EventIdType.NumbericChange, (NumericType)final, this.Entity.Id, (int)numericType);
		}
	}
}